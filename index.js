...
function inject(main, secondaries, start, end, isDefault) {
    var injectCount = 0
    var unresolved = []
    ...
    return !isDefault
        ? inject(main, secondaries, start, end, true)
        : parsed
}

function template(main, secondaries, start, end) {
    ...
    if(typeof secondaries === 'object' && secondaries !== null) {
        secondaries = ea(secondaries).map(function(e) {
            return inject(parseFormat(e), null, start, end)
        })
    }

    return inject(main, secondaries, start, end)
}
